...
class TMan extends N2N {
    ...
    constructor (options = {}, parent) {
        ...
        super( _.merge({ pid: 'tman-wrtc',
                         ...
                         ranking: (peerId) => (a, b) => {
                             const db = b.descriptor.x -
                                   this.partialView.getDescriptor(peerId).x;
                             const da = a.descriptor.x -
                                   this.partialView.getDescriptor(peerId).x;
    ...
    /**
     * Get a sample of the partial view. It can depends 
     * @param {string} peerId The identifier of the peer to exchange with.
    ...
    _getSample (peerId) {
        ...
        this.partialView.forEach( (epv, neighbor) => {
            epv.ages.forEach( (age) => {
                !_.isEmpty(epv.descriptor) && flatten.push(neighbor);
            });
        });
        // #2 replace all peerId occurrences by ours
        flatten = flatten.map( (neighbor) => {
            let d = {descriptor: this.options.descriptor};
            if (neighbor === peerId){
                d.peer = this.getInviewId();
            } else {
                d.descriptor = this.partialView.getDescriptor(neighbor);
                d.peer = neighbor;
            };
            return d;
        });
        ...
        flatten.sort( this.options.ranking(peerId) );
    ...
    _open (peerId) {
        ...
        // #1 check if it is already in the view.        
        this.partialView.addNeighbor(peerId);
        // #2 check if it has descriptors otherwise, ask for them.
    };
